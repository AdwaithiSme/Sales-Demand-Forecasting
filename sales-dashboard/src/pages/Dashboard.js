...
export default function Dashboard() {
  ...
  useEffect(() => {
    const fetchCategories = async () => {
      try {
        const res  = await fetch((process.env.REACT_APP_API_URL || 'http://127.0.0.1:8000') + '/available_categories?store_id=1', { headers: { 'X-API-Key': (process.env.REACT_APP_API_KEY || 'secret-token') } });
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      try {
        ...
        const headers  = { 'Content-Type': 'application/json', 'X-API-Key': (process.env.REACT_APP_API_KEY || 'secret-token') };
        const [sRes, fRes, aRes] = await Promise.all([
          fetch((process.env.REACT_APP_API_URL || 'http://127.0.0.1:8000') + '/dashboard/summary',  { method: 'POST', headers, body: payload }),
          fetch((process.env.REACT_APP_API_URL || 'http://127.0.0.1:8000') + '/predict',            { method: 'POST', headers, body: JSON.stringify({ store_id: 1, family: selectedFamily, months: 1 }) }),
          fetch((process.env.REACT_APP_API_URL || 'http://127.0.0.1:8000') + '/analyze_history',    { method: 'POST', headers, body: payload }),
        ]);
  ...
  const handleRetrain = async () => {
    ...
    try {
      const res = await fetch((process.env.REACT_APP_API_URL || 'http://127.0.0.1:8000') + '/retrain', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json', 'X-API-Key': (process.env.REACT_APP_API_KEY || 'secret-token') },
