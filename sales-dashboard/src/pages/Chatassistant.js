...
const PurchaseOrderWidget = ({ data, onOrderApproved, initialStatus, onStatusChange }) => {
  ...
  const handleAction = async (action) => {
    ...
    try {
      const res = await fetch((process.env.REACT_APP_API_URL || 'http://127.0.0.1:8000') + '/submit_order', {
        method: 'POST',
...
const ChatAssistant = () => {
  ...
  const sendMessage = async () => {
    ...
    try {
      const response = await fetch((process.env.REACT_APP_API_URL || 'http://127.0.0.1:8000') + '/chat', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json', 'X-API-Key': (process.env.REACT_APP_API_KEY || 'secret-token') },
        body: JSON.stringify({
          message: input,
