...
const Inventory = () => {
  ...
  useEffect(() => {
    fetch((process.env.REACT_APP_API_URL || 'http://127.0.0.1:8000') + '/available_categories?store_id=1', { headers: { 'X-API-Key': (process.env.REACT_APP_API_KEY || 'secret-token') } })
      .then(r => r.json())
      .then(d => {
  ...
  const getAdvice = async () => {
    setIsLoading(true);
    try {
      const res = await fetch((process.env.REACT_APP_API_URL || 'http://127.0.0.1:8000') + '/inventory', {
        method: 'POST',
